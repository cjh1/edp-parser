...
def _build_ana_rcp_parser():
    ...
    plate_id = pp.Literal('plate_id')  + separator +  pyparsing_common.integer
    plate_ids = pp.Literal('plate_ids')  + separator +  pyparsing_common.integer


    key_value = (
        platemap_keylist | run_ids | plate_id | plate_ids |
        key + separator + value
    )
